...
if platform == "win32":
    import msvcrt
    from ctypes import (
        wintypes, Structure, Union, c_void_p, POINTER, windll,
        WinError, cast, c_char_p, byref, c_longlong,
    )
    ...
    WriteFile.argtypes = [
        ...
        wintypes.LPDWORD, POINTER(OVERLAPPED),
    ]
    ...
    def pwrite(fd: typing.Union[int, wintypes.HANDLE], data: bytes, offset: int) -> int:
        ...
        WriteFile(
            handle, cast(c_char_p(data), wintypes.LPCVOID),
            len(data), byref(written), byref(overlapped),
        )
    ...
    DeviceIoControl.argtypes = [
        ...
        wintypes.LPVOID, wintypes.DWORD, POINTER(wintypes.DWORD), wintypes.LPVOID,
    ]
    ...
    SetFilePointerEx.argtypes = [
        wintypes.HANDLE, c_longlong, POINTER(c_longlong), wintypes.DWORD,
    ]
    ...
    def ftruncate(fd: typing.Union[int, wintypes.HANDLE], size: int) -> None:
        ...
        try:
            DeviceIoControl(
                handle, FSCTL_SET_SPARSE, None, 0,
                None, 0, byref(returned), None,
            )
        except OSError:
            logger.debug("File system does not support sparse files; using normal pre-allocation")
    ...
    from os import pwrite, ftruncate  # noqa: F401
