"""HTTP subworker edge-case tests — prevent file corruption and silent failure.

Uses a local aiohttp server to precisely control server behaviour.

Seams covered:
  S13a — Range correctness (no overlap, no gaps)
  S13b — Dynamic subworker reassignment
  S13c — Server degradation (200-for-range, 403, cf-mitigated, disk-full)
  S13d — Resume integrity (.ghd format)
  S13e — Request construction (headers, speed reporting)
  S13g — Stall recovery
"""
...
from multidownload import HttpStep, Subworker, TaskStatus, DownloadError, PermanentDownloadError, RangeNotSupportedError
...
def makeStep(
    ...
    *,
    fileSize: int,
    subworkerCount: int = 2,
    canUseRangeRequests: bool = True,
    userAgent: str = "",
    clientProfile: str = "raw",
) -> HttpStep:
    outputFile = str(tmpdir / "test.bin")
    return HttpStep(
        stepIndex=0,
        url=url,
        fileSize=fileSize,
...
async def runStep(step: HttpStep) -> list[int]:
...
class TestRangeCorrectness:
    ...
    def test_buildSubworkers_two_workers_no_overlap(self):
        step = HttpStep(stepIndex=0, url="", fileSize=1000,
    ...
    def test_buildSubworkers_one_byte_file(self):
        step = HttpStep(stepIndex=0, url="", fileSize=1,
    ...
    def test_buildSubworkers_clamps_to_filesize(self):
        step = HttpStep(stepIndex=0, url="", fileSize=3,
    ...
    async def test_download_produces_correct_file(self, server, tmpdir):
        ...
        step = makeStep(url, tmpdir, fileSize=1000, subworkerCount=2)
    ...
    async def test_single_subworker_correct(self, server, tmpdir):
        ...
        url = await server(rangeHandler(content))
        step = makeStep(url, tmpdir, fileSize=500, subworkerCount=1)
        step.setStatus(TaskStatus.RUNNING)
    ...
    async def test_many_subworkers_correct(self, server, tmpdir):
        ...
        url = await server(rangeHandler(content))
        step = makeStep(url, tmpdir, fileSize=100, subworkerCount=10)
        step.setStatus(TaskStatus.RUNNING)
...
class TestServerDegradation:
    ...
    async def test_403_raises_permanent_error(self, server, tmpdir):
        ...
        step = makeStep(url, tmpdir, fileSize=100, subworkerCount=1)
        step.setStatus(TaskStatus.RUNNING)
    ...
    async def test_cloudflare_mitigated_raises_permanent(self, server, tmpdir):
        async def cfMitigated(request):
            return web.Response(status=403, headers={"cf-mitigated": "challenge"}, text="Cloudflare")
    ...
    async def test_no_range_support_downloads_full(self, server, tmpdir):
        ...
        step = makeStep(url, tmpdir, fileSize=500, subworkerCount=1, canUseRangeRequests=False)
        step.setStatus(TaskStatus.RUNNING)
...
class TestRequestConstruction:
    ...
    async def test_effective_headers_include_user_agent(self, server, tmpdir):
        ...
        step = makeStep(url, tmpdir, fileSize=100, subworkerCount=2, userAgent="GhostTest/1.0")
        step.setStatus(TaskStatus.RUNNING)
        ...
        for hdrs in receivedHeaders:
            ...
            assert "GhostTest/1.0" in ua, f"User-Agent missing from request headers: {hdrs}"
...
class TestReassignment:
    ...
    def _makeStepWithSubworkers(self, fileSize=1_000_000, subworkerCount=2):
        step = HttpStep(stepIndex=0, url="", fileSize=fileSize,
    ...
    def test_skips_when_filesize_zero(self):
        step = HttpStep(stepIndex=0, url="", fileSize=0, subworkerCount=1, canUseRangeRequests=False)
        step.subworkers = [Subworker(index=0, start=0, end=-1)]
    ...
    async def test_reassignment_during_download(self, server, tmpdir):
        ...
        step = makeStep(url, tmpdir, fileSize=2_000_000, subworkerCount=2)
        step.setStatus(TaskStatus.RUNNING)
...
class TestResume:
    ...
    async def test_resume_sw0_done_sw1_pending(self, server, tmpdir):
        ...
        step = makeStep(url, tmpdir, fileSize=1000, subworkerCount=2, canUseRangeRequests=True)
        step.setStatus(TaskStatus.RUNNING)
    ...
    async def test_resume_with_increased_thread_count(self, server, tmpdir):
        """User raised subworkerCount from 2 to 4. Resume should split remaining."""
        content = makeFileContent(1000)
        url = await server(rangeHandler(content))
        ...
        step = makeStep(url, tmpdir, fileSize=1000, subworkerCount=4, canUseRangeRequests=True)
        step.setStatus(TaskStatus.RUNNING)
    ...
    async def test_resume_decreased_thread_count_is_noop(self, server, tmpdir):
        """User reduced subworkerCount. Existing subworkers survive."""
        content = makeFileContent(1000)
        url = await server(rangeHandler(content))
...
class TestRangeFallback:
    ...
    async def test_200_for_range_falls_back_to_single_stream(self, server, tmpdir):
        ...
        step = makeStep(url, tmpdir, fileSize=500, subworkerCount=2, canUseRangeRequests=True)
        step.setStatus(TaskStatus.RUNNING)
    ...
    async def test_200_for_range_single_subworker(self, server, tmpdir):
        ...
        url = await server(ignoreRangeHandler(content))
        step = makeStep(url, tmpdir, fileSize=200, subworkerCount=1, canUseRangeRequests=True)
        step.setStatus(TaskStatus.RUNNING)
...
class TestDiskFull:
    ...
    async def test_enospc_raises_task_error(self, server, tmpdir, monkeypatch):
        """pwrite raises ENOSPC — should bubble as DownloadError, not retry."""
        ...
        monkeypatch.setattr("multidownload._engine.pwrite", failing_pwrite)
...
class TestStallRecovery:
    ...
    async def test_stalled_stream_recovers_and_completes(self, server, tmpdir, monkeypatch):
        ...
        monkeypatch.setattr("multidownload._engine.STREAM_READ_TIMEOUT", 1)
